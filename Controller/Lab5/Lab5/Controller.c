...
int bitParser(Controller *self, int arg) {
  ...
  if (data & NORTH_ENTRY) {
    ...
    self->BridgePassedSameDir++; // Öka hur många bilar som har åkt i samma
                                 // riktning.
    AFTER(CURRENT_OFFSET() + SEC(BRIDGE_TIME), self, updateBridgeAmount,
          -1); // Minska antal bilar på bron när den kört över. (Tidsbaserat).
  }
...
int signalGreenLight(Controller *self, int arg) {
  ...
  AFTER(CURRENT_OFFSET() + MSEC(GREEN_LIGHT_TIME_MSEC), self, monitorCarEntry,
        0);
  return 0;
}
...
int monitorCarEntry(Controller *self, int arg) {
  if (self->BridgeAmount > 0) { // Om en bil har kört in på bron.
    ...
    AFTER(CURRENT_OFFSET() + SEC(TIME_QUEUE), self, dispatchNextCar,
          0); // Vänta innan nästa bil skickas.
  } else {
    AFTER(CURRENT_OFFSET(), self, monitorCarEntry,
